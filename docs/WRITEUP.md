...
**Two arms, same questions, same weights.** Twenty questions, ten covered by the
closed list and ten plainly outside it (weather, notice periods, football). The
baseline arm asks `google/gemma-4-E4B-it` directly — no routing, no retrieval,
no checking. The pipeline arm runs the nine steps above. Both against the same
vLLM box.

```
                                      model alone   our pipeline
out of corpus, answered / published        8/10           0/10
out of corpus, refused                    10/10          10/10   (routing)
in corpus, answered / published           10/10           4/10
in corpus, refused                         0/10           0/10
wall clock                                53.3s          91.5s
```

Read the first line on its own: the model alone answers eight of the ten
questions it has no source for, in the same register it uses when it does. It
declines only the two that are explicitly about live data (tomorrow's weather,
today's price per square metre). Routing refuses all ten, and refuses none of
the ten it does cover.

The pipeline published something on four of the ten covered questions. The other
six were retrieved, drafted, decomposed — then withheld entirely. That is the
design working as specified and still being wrong: the decomposer turns medRxiv
inline citation markers into claims of their own, which no passage can back, and
those fragments sink the whole answer. We report it rather than trim the set.

`metrics/README.md` holds the per-question table; `metrics/baseline.json` and
`metrics/pipeline.json` hold the raw runs. Both are regenerated by
`python scripts/compare_metrics.py`, so no figure here is typed by hand.
